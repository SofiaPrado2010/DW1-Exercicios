...
async function procure() {
    const id_livro = document.getElementById("inputId").value;
...
async function salvar() {
    let id_livro = livro ? livro.id_livro : parseInt(document.getElementById("inputId").value);
    ...
    if (!id_livro || !titulo || !autor || !ano || !genero || !paginas) {
        alert("Erro nos dados digitados");
        return;
    }

    const dadosLivro = { id_livro: id_livro, titulo: titulo, autor: autor, ano: ano, genero: genero, paginas };
...
function preparaListagem(vetor) {
    ...
    for (let i = 0; i < vetor.length; i++) {
        const linha = vetor[i];
        texto += `${linha.id_livro} - ${linha.titulo} - ${linha.autor} - ${linha.ano} - 
        ${linha.genero} - ${linha.paginas}<br>`;
    }
...
function mostrarDadosLivro(livro) {
    document.getElementById("inputId").value = livro.id_livro;
